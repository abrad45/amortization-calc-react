...
describe('useGetPaymentData', () => {
  ...
  describe('bailout guards', () => {
    const renderWith = (interestState) => {
      const store = createTestStore(interestState);

      return renderHook(() => useGetPaymentData(), {
        wrapper: ({ children }) => wrapper({ children, store }),
      });
    };

    it('should bail out when the payment exactly equals the interest', () => {
      // Emptying the payment field coerces to 0. At 0% interest the monthly
      // interest is also 0 — the balance never moves, and the old `>` guard
      // let this through and spun forever until the tab ran out of memory.
    ...
    it('should bail out on a negative payment', () => {
      const { result } = renderWith({
        balance: 10000,
        interestRate: 6.25,
        payment: -50,
    ...
    it('should bail out when an input is not a number', () => {
      // The reducer does `+value`, so non-numeric input arrives as NaN.
      // Left unguarded this rendered "$NaN" rows in the table.
      expect(renderWith({ balance: 'abc' }).result.current).toEqual({
        error: 'insufficient_payment',
      });
      expect(renderWith({ interestRate: 'abc' }).result.current).toEqual({
        error: 'insufficient_payment',
      });
      expect(renderWith({ payment: 'abc' }).result.current).toEqual({
        error: 'insufficient_payment',
      });
    });

    it('should bail out rather than run past the payment cap', () => {
      // $100k at 0% paying $50/mo pays off honestly — in 2000 months. The
      // interest guard never trips here, so only the cap stops it.
      const { result } = renderWith({
        balance: 100000,
        interestRate: 0,
        payment: 50,
    ...
    it('should still allow a long-but-plausible payoff under the cap', () => {
      // Same shape, 1000 months — under the 1200 cap, so it must complete.
      const { result } = renderWith({
        balance: 100000,
        interestRate: 0,
        payment: 100,
